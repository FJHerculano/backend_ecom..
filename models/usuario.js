...
UsuarioSchema.methods.validarSenha = function(password){
  const hash = crypto.pbkdf2Sync(password, this.salt, 10000, 512, "sha512").toSting("hex");
  return hash === this.hash;
};
//de 15 em 15 dias o token expira e é gerado um novo 
UsuarioSchema.methods.gerarToken = function(){
  ...
  return jwt.sign({
    ...
    email: this.email,
    nome:this.nome,
    exp: parseFloat(exp.getTime()/100,10)
  }, secret);
};
